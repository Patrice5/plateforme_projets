    ...
    if (empty($errors)) {
        try {
            ...
            if (!empty($nouveau_mot_de_passe)) {
                ...
                $stmt = $pdo->prepare("
                    UPDATE utilisateurs 
                    SET nom = ?, prenom = ?, email = ?, niveau = ?, filiere_id = ?, mot_de_passe_hash = ?
                    WHERE id = ?
                ");
                $stmt->execute([$nom, $prenom, $email, $niveau, $filiere_id, $mot_de_passe_hash, $user_id]);
            } else {
                $stmt = $pdo->prepare("
                    UPDATE utilisateurs 
                    SET nom = ?, prenom = ?, email = ?, niveau = ?, filiere_id = ?
                    WHERE id = ?
                ");
                $stmt->execute([$nom, $prenom, $email, $niveau, $filiere_id, $user_id]);
            }
...
try {
    ...
    error_log("Erreur profil étudiant: " . $e->getMessage());
    $user = null;
}
...
<head>
    ...
    <title>Mon Profil - Étudiant - UNB-ESI</title>
                ...
                <div class="p-4 border-t">
                    <div class="flex items-center space-x-3 mb-6">
                        ...
                        <div>
                            ...
                            <p class="text-xs text-gray-400"><?php echo htmlspecialchars($user['filiere_nom'] ?? 'Aucune filière'); ?></p>
                        </div>
                    </div>
                    
                    <nav class="space-y-2">
                        <a href="dashboard_etudiant.php" class="nav-link flex items-center space-x-3 p-2 rounded-lg text-gray-700 hover:bg-gray-100">
                        ...
                        <a href="profile.php" class="nav-link flex items-center space-x-3 p-2 rounded-lg bg-indigo-50 text-indigo-700">
        ...
        <aside class="sidebar hidden md:flex md:flex-col w-64 bg-white shadow-lg">
            ...
            <div class="flex-1 overflow-y-auto">
                <div class="p-4">
                    ...
                    <nav class="space-y-1">
                        <a href="dashboard_etudiant.php" class="nav-link flex items-center space-x-3 p-3 rounded-lg text-gray-700 hover:bg-gray-100">
                            <i class="fas fa-tachometer-alt w-5 text-center"></i>
                            <span>Tableau de bord</span>
                        </a>
                        ...
                        <a href="profile.php" class="nav-link flex items-center space-x-3 p-3 rounded-lg bg-indigo-50 text-indigo-700">
                            <i class="fas fa-user w-5 text-center"></i>
                            <span>Mon Profil</span>
                        </a>
                    </nav>
                            ...
                            <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                                ...
                                <div>
                                    ...
                                    >
                                        ...
                                        <option value="L1-TC1" <?php echo ($user['niveau'] == 'L1-TC1') ? 'selected' : ''; ?>>L1-TC1</option>
                                        <option value="L2-TC2" <?php echo ($user['niveau'] == 'L2-TC2') ? 'selected' : ''; ?>>L2-TC2</option>
                                        <option value="L3-ISI" <?php echo ($user['niveau'] == 'L3-ISI') ? 'selected' : ''; ?>>L3-ISI</option>
                                        <option value="L3-IRS" <?php echo ($user['niveau'] == 'L3-IRS') ? 'selected' : ''; ?>>L3-IRS</option>
                                    </select>
                                </div>
                            </div>
...
<div>
            ...
            <div class="flex flex-col sm:flex-row gap-4 pt-6 border-t">
                <button 
                    type="submit" 
                    class="flex-1 bg-indigo-600 hover:bg-indigo-700 text-white font-medium py-3 px-6 rounded-lg transition duration-200 flex items-center justify-center"
                >
